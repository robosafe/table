#!/usr/bin/env python
"""
Implements assertion 2: if 'flag1==1' then check 'flag2==1' and check time difference
...
class Flag2(smach.State):
	...
	if receivedflag2 == 1:
		if (time.time()-start) < 10:
			stats.write('Assertion 2 at test '+ str(fileno) +': Passed at global time '+ str(time.time()-globaltime) +'\n')
		else:
			stats.write('Assertion 2 at test '+ str(fileno) +': Failed at global time '+ str(time.time()-globaltime) +'\n')
		receivedflag2 = 0
		return 'outcome1'
        else:
		return 'outcome2'
