...
def interface(group_variables):
  ...
  group_variable_values = group.get_current_joint_values() #Start of trajectory to break plans to limit speed
  ...
  try:
  	...
  	for j,traj in enumerate(the_traj):
  		temp_plan = [traj.positions[0],traj.positions[1],-1000.0,-1000.0, traj.positions[2],-1000.0, traj.positions[3], traj.positions[4], traj.positions[5], traj.positions[6], traj.positions[7], traj.positions[8]] 
